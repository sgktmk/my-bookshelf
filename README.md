...
- ☁️ **GitHub直接保存**: ブラウザから`data/library.json`を直接コミット（エクスポート＆手動push不要）
- 📥 **多様な登録手段**: タイトル検索・ISBNバーコードスキャン・ASIN/ISBN自動取得・Kindleインポート・手動追加
- 📚 **形式管理**: 紙の本・Kindle・EPUB・PDFを区別して登録
...
**方法2: タイトル・著者で検索して追加**
1. 「➕ 本を追加」ボタンをクリック
2. 「🔍 検索して追加」タブでタイトルや著者を検索（Google Books）
3. 候補リストから「➕ 追加」をクリック（書誌情報・表紙は自動で埋まる）

**方法3: バーコードスキャンで追加（紙の本）**
1. 「➕ 本を追加」→「📷 バーコード」タブ
2. 本の裏表紙のISBNバーコード（978で始まる方）をカメラにかざす
3. 読み取り後、OpenBD / Google Books から書籍情報を自動取得

**方法4: ASIN / ISBN / AmazonのURLで追加**
1. 「➕ 本を追加」→「🆔 ASIN / ISBN」タブ
2. ASIN・ISBN、またはAmazonの商品URLを貼り付けて「📥 自動取得」
3. 不足分だけ手動で補完して追加

いずれの方法でも「📚 本の形式」（紙・Kindle・EPUB・PDF）を指定できます。
「自動判定」の場合、ISBNは紙の本、Bで始まるASINはKindleとして登録されます。
